...
void ExecutionConfig::set_default() {
    register_property<PropertyVisibility::PUBLIC>(
        ...
        std::make_tuple(ov::enable_lp_transformations, false));

    register_property<PropertyVisibility::INTERNAL>(
        std::make_tuple(ov::max_dynamic_batch, 1),
        std::make_tuple(ov::queue_type, QueueTypes::out_of_order),
        std::make_tuple(ov::optimize_data, false),
        std::make_tuple(ov::enable_memory_pool, true),
        std::make_tuple(ov::allow_static_input_reorder, false),
        std::make_tuple(ov::custom_outputs, std::vector<std::string>{}),
        std::make_tuple(ov::dump_graphs, ""),
        // std::make_tuple(ov::force_implementations, ImplForcingMap{}),
        std::make_tuple(ov::partial_build_program, false),
        std::make_tuple(ov::allow_new_shape_infer, false),
        std::make_tuple(ov::use_only_static_kernels_for_dynamic_shape, false),
        std::make_tuple(ov::buffers_preallocation_ratio, 1.1f));
}
