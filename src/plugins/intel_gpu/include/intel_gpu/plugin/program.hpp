...
class Program final {
public:
    Program(std::shared_ptr<ov::Model> model, cldnn::engine& engine, const ExecutionConfig& config,
    ...
    std::shared_ptr<cldnn::program> get_compiled_program();
    ...
    using factory_t = std::function<void(Program&, const std::shared_ptr<ov::Node>&)>;
...
void CreateCustomOp(Program& p, const std::shared_ptr<ov::Node>& node, CustomLayerPtr customLayer);
void CreateUnaryEltwiseOp(Program& p, const std::shared_ptr<ov::Node>& node,
