...
ov::Output<ov::Node> reshape_input_for_matmul(const ov::Output<ov::Node>& input_node,
    ...
    const auto new_shape_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {new_shape.size()}, new_shape);
    const auto reshaped_input_op = std::make_shared<ov::opset7::Reshape>(input_node, new_shape_const, false);
...
LabelDimMap compute_label_dim_map(const ov::Rank& input_rank,
    ...
    const auto labels = ov::opset7::Einsum::extract_labels(input_subscript);
...
void transpose_input(ov::OutputVector& input_nodes, std::vector<std::string>& input_subscripts, const std::string& required_subscript, size_t input_ind,
    ...
    const auto labels = ov::opset7::Einsum::extract_labels(input_subscript);
    const auto required_labels = ov::opset7::Einsum::extract_labels(required_subscript);
    ...
    const auto permutation_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {permutation.size()}, permutation);
    const auto transpose = std::make_shared<ov::opset7::Transpose>(input_node, permutation_const);
...
void reduce_input(EinsumDecomposition *einsum_decompose_ptr,
    ...
    const auto labels = ov::opset7::Einsum::extract_labels(input_subscripts[input_ind]);
    ...
    const auto axes_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {reduced_axes.size()}, reduced_axes_vec);
    const auto reduce_sum = einsum_decompose_ptr->register_new_node<ov::opset7::ReduceSum>(input_node, axes_const, false);
...
void broadcast_input(ov::OutputVector& inputs,
    ...
    const auto new_shape_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {new_shape.size()}, new_shape);
    const auto broadcast = std::make_shared<ov::opset7::Broadcast>(input, new_shape_const, ov::op::BroadcastType::NUMPY);
...
ov::Output<ov::Node> build_identity(const ov::Output<ov::Node>& input_node,
    ...
    const auto identity = ov::opset7::Constant::create(input_node.get_element_type(), identity_shape, identity_values);
...
ov::Output<ov::Node> build_multi_identity(EinsumDecomposition* einsum_decompose_ptr,
    ...
    for (size_t label_ind = 1; label_ind < repeated_labels.size(); ++label_ind) {
        ...
        const auto mul = std::make_shared<ov::opset7::Multiply>(multi_identity, identity, ov::op::AutoBroadcastType::NUMPY);
...
void extract_diagonal(EinsumDecomposition* einsum_decompose_ptr,
    ...
    const auto mul = std::make_shared<ov::opset7::Multiply>(input_node, multi_identity, ov::op::AutoBroadcastType::NUMPY);
    subgraph_nodes.insert(subgraph_nodes.end(), {mul});

    const std::vector<int64_t> reduced_axes_vec{reduced_axes.cbegin(), reduced_axes.cend()};
    const auto axes_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {reduced_axes.size()}, reduced_axes_vec);
    const auto reduce_sum = std::make_shared<ov::opset7::ReduceSum>(mul->output(0), axes_const, false);
    subgraph_nodes.insert(subgraph_nodes.end(), {axes_const, reduce_sum});
...
void contract_two_inputs(EinsumDecomposition* einsum_decompose_ptr,
    ...
    if (reduced_labels_inds1.empty()) {
        ...
        const auto separate_labels1 = ov::opset7::Einsum::extract_labels(separate_part1);
        const auto separate_labels2 = ov::opset7::Einsum::extract_labels(separate_part2);
        ...
        // multiply both operands with broadcasting
        auto mul = std::make_shared<ov::opset7::Multiply>(unsqueeze_output1, unsqueeze_output2, ov::op::AutoBroadcastType::NUMPY);
    ...
    const auto matmul = std::make_shared<ov::opset7::MatMul>(matmul_operand1, matmul_operand2, transpose_a, transpose_b);
    ...
    const auto new_shape_const = ov::opset7::Constant::create(ov::element::Type_t::i64, ov::Shape {back_shape.size()}, back_shape);
    const auto reshape_result_op = std::make_shared<ov::opset7::Reshape>(matmul->output(0), new_shape_const, false);
...
EinsumDecomposition::EinsumDecomposition() {
    ...
    const auto einsum = ov::pass::pattern::wrap_type<ov::opset7::Einsum>();
