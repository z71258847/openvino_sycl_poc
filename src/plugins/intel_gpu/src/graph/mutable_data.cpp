...
event::ptr mutable_data_inst::set_output_memory(memory::ptr mem_new, bool check, size_t idx) {
    ...
    if (_node != nullptr) {
        ...
        const auto& mem_orig = output_memory(idx);
...
void mutable_data_inst::save(cldnn::BinaryOutputBuffer& ob) const {
    ...
    allocation_type _allocation_type = get_output(0).get_allocation_type();
    ...
    if (_allocation_type == allocation_type::usm_host || _allocation_type == allocation_type::usm_shared) {
        ...
        mem_lock<char, mem_lock_type::read> lock{output_memory_ptr(0), get_node().get_program().get_stream()};
...
void mutable_data_inst::load(BinaryInputBuffer& ib) {
    ...
    OPENVINO_ASSERT(get_output(0).allocated(), "[GPU] Output memory should be allocated before importing data.");
    allocation_type _allocation_type = get_output(0).get_allocation_type();

    if (_allocation_type == allocation_type::usm_host || _allocation_type == allocation_type::usm_shared) {
        ib >> make_data(output_memory_ptr(0)->buffer_ptr(), data_size);
    } else {
        std::vector<uint8_t> _buf;
        _buf.resize(data_size);
        ib >> make_data(_buf.data(), data_size);
        output_memory_ptr(0)->copy_from(get_network().get_stream(), _buf.data());
    }
}
