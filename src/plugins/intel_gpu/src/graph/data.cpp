...
void data_inst::save(cldnn::BinaryOutputBuffer& ob) const {
    parent::save(ob);
    ob << get_mem_manager(0).get_layout();

    const auto _allocation_type = get_mem_manager(0).get_allocation_type();
    ...
    if (_allocation_type == allocation_type::usm_host || _allocation_type == allocation_type::usm_shared) {
        ...
        get_mem_manager(0).get_memory()->copy_to(get_network().get_stream(), _buf.data());
...
void data_inst::load(BinaryInputBuffer& ib) {
    ...
    if (!get_network().is_primary_stream()) {
        get_mem_manager(0).set_memory(ib.getConstData(_network.get_local_id(), id()));
        auto pos = ib.tellg();
        pos += data_size;
        ib.seekg(pos);
    } else {
        get_mem_manager(0).allocate(output_layout, _allocation_type, false);
        auto mem = get_mem_manager(0).get_memory();
        if (_allocation_type == allocation_type::usm_host || _allocation_type == allocation_type::usm_shared) {
