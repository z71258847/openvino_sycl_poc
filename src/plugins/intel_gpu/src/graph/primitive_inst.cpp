...
void primitive_inst::update_shape() {
    ...
    if (!_network.shape_changed() && !_node.is_type<gather_nonzero>())
        return;

    for (size_t i = 0; i < _deps.size(); i++) {
        _impl_params->input_layouts[i] = _deps[i]->_impl_params->output_layout;
    }

    std::map<size_t, memory::ptr> memory_deps = _node.get_const_memory_deps();
    ...
    GPU_DEBUG_IF(debug_config->verbose >= 4) {
        GPU_DEBUG_COUT << id() << " update shape: was: " << _impl_params->output_layout << " now: " << new_layout << std::endl;
    }
...
void primitive_inst::update_impl() {
    ...
    if (!_node.is_type<data>() && !(_node.is_type<mutable_data>() && _node.get_dependencies().empty())) {
        ...
        //     }
        // } else {
            PRINT_TIME(_impl = std::move(_node.type()->choose_impl(_node, *_impl_params)));
            PRINT_TIME(_network.get_program()->compile());
            PRINT_TIME(_impl->init_kernels());
        // }
...
event::ptr primitive_inst::execute(const std::vector<event::ptr>& events) {
    ...
    if (_impl_params->output_layout.is_dynamic())
