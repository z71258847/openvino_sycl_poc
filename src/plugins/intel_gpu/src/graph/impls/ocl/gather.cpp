...
public:
    static kernel_params_t get_kernel_params(const kernel_impl_params& impl_param) {
        ...
        auto in_rank = impl_param.get_input_layout(0).get_partial_shape().size();
        auto out_rank = impl_param.get_output_layout(0).get_partial_shape().size();
        ...
        ov::PartialShape new_output_shape = output_layout.get_partial_shape();;
        if (new_output_shape.size() < 4) {
            new_output_shape.insert(new_output_shape.begin(), 4 - new_output_shape.size(), ov::Dimension(1));
            output_layout = layout(new_output_shape, impl_param.get_output_layout(0).data_type, format::get_default_format(new_output_shape.size()));
        }
