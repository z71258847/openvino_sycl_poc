...
protected:
    kernel_arguments_data get_arguments(const typed_primitive_inst<fully_connected>& instance) const override {
        ...
        if (!desc->compression_scale.empty())
            args.inputs.push_back(instance.dep_memory_ptr(in_id++));

        if (!desc->compression_zero_point.empty())
            args.inputs.push_back(instance.dep_memory_ptr(in_id));
...
public:
    static kernel_params_t get_kernel_params(const kernel_impl_params& impl_param, bool is_shape_agnostic = false) {
        ...
        auto get_fc_input_layouts = [primitive](const std::vector<layout>& input_layouts, bool allow_new_shape_infer) {
            ...
            bool has_zp = !primitive->compression_zero_point.empty();
            bool has_scale = !primitive->compression_scale.empty();
        ...
        bool commpressed = !primitive->compression_scale.empty();
        bool with_zp = !primitive->compression_zero_point.empty();
        if (commpressed) {
