...
ESIMD_INLINE void GEMV_Int4Weight_FP32InOutNx16Temp_largeGRF_block(uint8_t* a, uint8_t* b, uint8_t* c, uint8_t* d, nd_item<1>& ndi) {
...
#pragma unroll
  for (int k = 0; k < K_DIM_DIV_4096; k++) {
    ...
    {
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k) =
      ...
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*1) =
        __ESIMD_ENS::lsc_block_load<
        uint8_t,
        128,
        __ESIMD_ENS::lsc_data_size::default_size,
        __ESIMD_ENS::cache_hint::cached,
        __ESIMD_ENS::cache_hint::cached>((uint8_t*)b + offsetB + 1*64 * sizeof(fp16));

      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*2) =
        __ESIMD_ENS::lsc_block_load<
        uint8_t,
        128,
        __ESIMD_ENS::lsc_data_size::default_size,
        __ESIMD_ENS::cache_hint::cached,
        __ESIMD_ENS::cache_hint::cached>((uint8_t*)b + offsetB + 2*64 * sizeof(fp16));
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*3) =
        __ESIMD_ENS::lsc_block_load<
        uint8_t,
        128,
        __ESIMD_ENS::lsc_data_size::default_size,
        __ESIMD_ENS::cache_hint::cached,
        __ESIMD_ENS::cache_hint::cached>((uint8_t*)b + offsetB + 3*64 * sizeof(fp16));
    } 
    else 
    {
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k) = 0;
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*1) = 0;
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*2) = 0;
      bb_fp16.template bit_cast_view<unsigned char>().template select<128, 1>(512 * k + 128*3) = 0;
    }

    offsetB += 4096 * sizeof(fp16);
  }
  bb.select<1280, 1>(0) = bb_fp16.select<1280, 1>(0);
...
#pragma unroll
    for (int k = 0; k < 4; k++) {
      bb.select<64, 1>(64 * k) = slm_block_load<float, 64>(slmSumPhase1LoadOffset + 64 * k * sizeof(float));
    }
...
#pragma unroll
    ...
    __ESIMD_ENS::lsc_block_store<
      ...
      __ESIMD_ENS::cache_hint::write_back>((float*)c + outputOffset + hh * 16, bb.select<16, 1>(0));
  }
}