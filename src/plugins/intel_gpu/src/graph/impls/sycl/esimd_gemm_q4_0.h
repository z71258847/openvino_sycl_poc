...
#pragma unroll
  ...
  // //Yuchen debug
  // cc_fp16.select<16, 1>(0) = bb.template bit_cast_view<uint8_t>().template
  // select<16, 1>(0);
  ...
  //           __ESIMD_ENS::cache_hint::write_back>((fp16*)c + 0,
  //           cc_fp16.select<16, 1>(0));
...
#pragma unroll
      for (int k = 0; k < 4; k++) {
        bb.select<64, 1>(64 * k) =
            slm_block_load<float, 64>(slmOffset + k * 64 * sizeof(64));
      }
...
#pragma unroll
        ...
        __ESIMD_ENS::lsc_block_store<
            ...
            __ESIMD_ENS::cache_hint::write_back,
            __ESIMD_ENS::cache_hint::write_back>(
            (float*)c + offsetC, cc_fp16.select<16, 1>(0));
...
ESIMD_INLINE void gemmReduce768WeightsQ40InputFp16_ipex(
  ...
  constexpr uint32_t baseOffsetInc8[8] = {0, 1, 2, 3, 4, 5, 6, 7};
  constexpr uint32_t baseOffsetInc4[4] = {0, 1, 2, 3};
  __ESIMD_NS::slm_init(24 * 8 * 16 * sizeof(fp16) + 16 * 32 * sizeof(fp16));
  int hh = ndi.get_local_linear_id(); // [0, 64)
  int h = ndi.get_group(0); // [0, (row + 15) / 16)
  int v = reduceIdx; // [0, (row + 15) / 16)
  int outputRow = ndi.get_group_range(0) * 16;
...
#pragma unroll
    for (int k = 0; k < 8; k++) {
      slm_block_store<fp16, 16>(
          (hh * 16 + k * 16 * 24) * sizeof(fp16), cc.select<16, 1>(16 * k));
    }
    ...
    if (hh < 8) {
      if (8 * nn + hh < tokenSize) {
        uint32_t slmOffset = hh * 16 * 24 * sizeof(fp16);
        if (v != 0) {
          cc_fp16.select<16, 1>(0) = __ESIMD_ENS::lsc_block_load<
              fp16,
              16,
...
#pragma unroll
        for (int k = 0; k < 4; k++) {
          bb.template bit_cast_view<fp16>().template select<64, 1>(64 * k) =
              slm_block_load<fp16, 64>(slmOffset);
          slmOffset += 64 * sizeof(fp16);
        }
