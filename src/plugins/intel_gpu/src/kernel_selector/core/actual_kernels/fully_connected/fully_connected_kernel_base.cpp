...
KernelsData FullyConnectedKernelBase::GetCommonKernelsData(const Params &params,
    ...
    kd.update_kernels_func = [this](const Params& params, KernelData& kd) {
        ...
        if (prim_params.layerID == "matmul:MatMul_1161") {
            std::cerr << "update ref " << params.layerID << " dispatch data: gws: " << gws[0] << " " << gws[1] << " " << gws[2] << "\n";
            std::cerr << "update ref " << params.layerID << " dispatch data: lws: " << lws[0] << " " << lws[1] << " " << lws[2] << "\n";
        }
    };
