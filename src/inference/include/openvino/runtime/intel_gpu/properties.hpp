...
public:
    ...
    explicit MemoryType(const std::string& smask) {
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            if (smask.find(allocation_type) != std::string::npos) {
                mask |= (static_cast<uint8_t>(i) & allocation_type_mask) << 4;
            }
        }
    }
    ...
    friend inline MemoryType operator|(MemoryType a, MemoryType b);
    friend inline MemoryType operator&(MemoryType a, MemoryType b);
private:
    uint8_t mask;

    const char* storage_types[2] = {
        "GPU_BUFFER",
        "GPU_SURFACE"
    };

    const char* allocation_types[2] = {
        "HOST",
        "REMOTE"
    };

    static const uint8_t storage_type_mask = 0x01;
    static const uint8_t allocation_type_mask = 0x10;
};

inline MemoryType operator|(MemoryType a, MemoryType b) {
    return static_cast<MemoryType>(a.mask | b.mask);
}
inline MemoryType operator&(MemoryType a, MemoryType b) {
    return static_cast<MemoryType>(a.mask & b.mask);
}
