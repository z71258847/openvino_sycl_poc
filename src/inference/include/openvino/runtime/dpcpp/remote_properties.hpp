...
inline std::istream& operator>>(std::istream& is, ContextType& context_type) {
    ...
    is >> str;
    if (str == "DPCPP") {
        context_type = ContextType::DPCPP;
    } else {
 ...
 * in a shared context or shared memory blob parameter map
 */
static constexpr Property<gpu_handle_param> context_handle{"CONTEXT_HANDLE"};
