...
void remove_redundant_reorders::run(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        if (i_layout.format == format::b_fs_yx_fsv16 && o_layout.format == format::bfyx && !r_node.is_output() &&
            ...
            pad_lo.set_batch(0, i_layout.data_padding.lower_size().batch(0));
            pad_hi.set_batch(0, i_layout.data_padding.upper_size().batch(0));

            pad_lo.set_feature(0, i_layout.data_padding.lower_size().feature(0));
            pad_hi.set_feature(0, i_layout.data_padding.upper_size().feature(0));

            if (i_layout.size.feature(0) % 16 != 0) {
                pad_hi.set_feature(0, pad_hi.feature(0) + 16 - i_layout.size.feature(0) % 16);
            }
        ...
        if (ident.first) {  // no need of reshape
            if (r_node.is_output()) {
                ...
                p.add_optimized_primitive_info(r_node.get_dependency(0).get_primitive()->output_ids[0], {r_node.get_primitive()->output_ids[0]});
            } else {
                p.add_optimized_primitive_info(r_node.get_primitive()->output_ids[0]);
            }
